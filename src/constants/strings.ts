class Strings {
    static readonly COLOR_ERROR = "red";
    static readonly COLOR_SUCCESS = "green";
    static readonly COLOR_WARNING = "yellow";
    static readonly USER_REGISTERED = "The user was registered successfully.";
    static readonly MERCHANT_REQUEST_REGISTERED = "The request to become a merchant was registered successfully.";
    static readonly FIELD_MISSING_IN_DATE_RANGE = "field missing in date range. Please check both fields are filled to search by date range";
    static readonly ERROR_MESSAGE = "There was an issue with the request. Please try again later.";
    static readonly TOGGLE_MESSAGE = "toggle"
    static readonly LETS_VERIFY_EMAIL = "Let's verify your email address.";
    static readonly UNVERIFIED_USER = "The user is no verified yet. Please contact with your admin";
    static readonly INCORRECT_CREDENTIALS =  "Credential Error. Verify User and password";
    static readonly PASSWORD_UPDATED =  "Password updated successfully";
    static readonly MAIL_SEND = "Mail sent"

}
export default Strings;
